...
func NewHTTPServer(agent *Agent, proto, network, addr string) (*HTTPServer, error) {
	cfg := agent.config
	var l net.Listener
	var err error
	switch proto {
	case "http":
		...
		default:
			return nil, fmt.Errorf("http: invalid proto/network %s/%s", proto, network)
		}
	case "https":
		switch network {
		case "tcp":
			...
			l, err = listenHTTPS(addr, tlsConfig)
		default:
			return nil, fmt.Errorf("http: invalid proto/network %s/%s", proto, network)
		}
	default:
		return nil, fmt.Errorf("http: invalid proto %s", proto)
	}

	if err != nil {
		return nil, err
	}
	srv := &HTTPServer{agent, agent.logger, &http.Server{Addr: addr}}
	srv.setHandler(cfg.EnableDebug)
	// todo(fs): we should catch the error from Serve() and act upon it
	go srv.Serve(l)
	return srv, nil
}
...
// setHandler is used to attach our handlers to the mux
func (s *HTTPServer) setHandler(enableDebug bool) {
...
func listenHTTPS(addr string, tlsConfig *tls.Config) (net.Listener, error) {
	l, err := net.Listen("tcp", addr)
	if err != nil {
		return nil, err
	}
	l = tls.NewListener(tcpKeepAliveListener{l.(*net.TCPListener)}, tlsConfig)
	return l, nil
}

func listenUnix(path string, perm UnixSocketConfig) (net.Listener, error) {
